...
var _ = g.Describe("[sig-scheduling][Feature:DRA-Example][Suite:openshift/dra-example][Serial][Skipped:Disconnected]", func() {
	...
	g.BeforeEach(func(ctx context.Context) {
		...
		builder = dracommon.NewResourceBuilder(oc.Namespace(), dracommon.DriverConfig{
			...
			DefaultClass:     defaultDeviceClass,
			RequestName:      deviceRequestName,
			ContainerImage:   image.ShellImage(),
			ContainerCommand: []string{"sh", "-c", "echo DRA device allocated && sleep infinity"},
			LongRunCommand:   []string{"sh", "-c", "while true; do echo DRA device active; sleep 60; done"},
		})
